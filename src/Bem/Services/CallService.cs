...
public sealed class CallService : ICallService
{
    ...
    /// <inheritdoc/>
    public async Task<CallRetrieveTraceResponse> RetrieveTrace(
        CallRetrieveTraceParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.RetrieveTrace(parameters, cancellationToken)
            .ConfigureAwait(false);
        return await response.Deserialize(cancellationToken).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public Task<CallRetrieveTraceResponse> RetrieveTrace(
        string callID,
    ...
    )
    {
        ...
        return this.RetrieveTrace(parameters with { CallID = callID }, cancellationToken);
    }
}
...
public sealed class CallServiceWithRawResponse : ICallServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<CallRetrieveTraceResponse>> RetrieveTrace(
        CallRetrieveTraceParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        ...
        HttpRequest<CallRetrieveTraceParams> request = new()
        {
            Method = HttpMethod.Get,
            Params = parameters,
        };
        ...
        return new(
            ...
            async (token) =>
            {
                var deserializedResponse = await response
                    .Deserialize<CallRetrieveTraceResponse>(token)
                    .ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
    ...
    /// <inheritdoc/>
    public Task<HttpResponse<CallRetrieveTraceResponse>> RetrieveTrace(
        string callID,
        CallRetrieveTraceParams? parameters = null,
        CancellationToken cancellationToken = default
    )
